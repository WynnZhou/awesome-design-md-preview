...
components:
  ...
  text-link-on-dark:
    ...
    typography: "{typography.body}"
  global-nav:
    backgroundColor: "{colors.surface-black} on dark pages, rgba(250, 250, 252, .8) on light pages"
    textColor: "{colors.on-dark} on dark pages, rgba(0, 0, 0, .8) on light pages"
...
- **Pure Black** (`{colors.surface-black}` — #000000): Reserved for true void — video player backgrounds, edge-to-edge photographic overlays, the global nav bar background on dark pages.
...
- **Body On Dark** (`{colors.body-on-dark}` — #ffffff): All text on dark tiles and on the dark-page global nav bar.
...
**`global-nav`** — Persistent, ultra-thin nav bar pinned to the top of every page. It has two
...
> except 6 passage(s) about the global nav, corrected against apple.com's own
